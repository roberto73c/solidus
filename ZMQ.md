# The Current/Future Status of ZMQ in Solidus

## ZMQ Pub/Sub
Client `ZMQ_SUB` sockets must "subscribe" to topics before it receives any data.
This allows filtering on the server side, so network traffic is reduced. Solidus
...
matching supported by both Solidus and ZMQ. The `format`, `context` and `event`
...
The Solidus daemon will ensure that events prefixed by `chain` will be sent in
...
chain instead of the last block. The Solidus daemon also ensures that
...
The Solidus daemon will send a `txpool_add` pub exactly once for each
