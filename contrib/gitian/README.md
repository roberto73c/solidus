...
*Setup instructions for a Gitian build of Solidus.*

Gitian is the deterministic build process that is used to build the Solidus CLI
...
```bash
cp solidus/contrib/gitian/gitian-build.py .
```
...
gpg --detach-sign ${VERSION}-linux/${GH_USER}/solidus-linux-*-build.assert
gpg --detach-sign ${VERSION}-win/${GH_USER}/solidus-win-*-build.assert
gpg --detach-sign ${VERSION}-osx/${GH_USER}/solidus-osx-*-build.assert
gpg --detach-sign ${VERSION}-android/${GH_USER}/solidus-android-*-build.assert
```
...
```bash
./gitian-build.py --detach-sign --no-commit --url https://github.com/solidusmooo-solidus/bitsolidus -b solidusmooo 1f5680c8db8f4cc7acc04a04c724b832003440fd
```
