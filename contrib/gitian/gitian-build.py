...
gsigs = 'https://github.com/solidus-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('solidus'):
        subprocess.check_call(['git', 'clone', args.url, 'solidus'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/solidus/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'solidus='+args.commit, '--url', 'solidus='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/solidus-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/solidus/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/solidus/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/solidus-project/solidus', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/solidus', exist_ok=True)
    os.chdir('builder/inputs/solidus')
    if args.pull:
