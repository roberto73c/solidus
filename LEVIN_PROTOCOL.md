...
used by Solidus. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Solidus's
...
> This document does not currently list all data being sent by the solidus
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Solidus p2p traffic!


## Header
This header is sent for every Solidus p2p message.
...
Solidus when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Solidus specific
...
### Requests
Requests are the basis of the admin protocol for Solidus. The `Q` bit must be
